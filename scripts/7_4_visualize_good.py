...
VISUALIZATION: GOOD vs NOT_GOOD
================================

Creates separate visualizations for:
1. good predictions (correct & wrong)
2. not_good predictions (correct & wrong)

Run: python -u scripts/7_4_visualize_good.py
...
def plot_class_samples(dataset, results, class_idx, class_name, output_path):
    ...
    total_rows = n_correct + n_wrong + 2
...
def main():
    print("=" * 70)
    print("  VISUALIZATION: GOOD vs NOT_GOOD")
    ...
    # 1. good predictions
    print("\n  [1] good predictions...", flush=True)
    g_correct, g_wrong = plot_class_samples(
        test_dataset, results, 
        class_idx=1, class_name='good',
        output_path=os.path.join(VIS_DIR, 'good_predictions.png')
    )
    print(f"      Correct: {g_correct}, Wrong: {g_wrong}")
    
    # 2. not_good predictions
    print("\n  [2] not_good predictions...", flush=True)
    n_correct, n_wrong = plot_class_samples(
        test_dataset, results,
        class_idx=0, class_name='not_good',
        output_path=os.path.join(VIS_DIR, 'not_good_predictions.png')
    )
    ...
    print(f"\n  📁 Output: {VIS_DIR}")
    print(f"      - good_predictions.png")
    print(f"      - not_good_predictions.png")
