...
OUTPUT_DIR = r"D:\Research\Cancer_Cell_Analysis\output\Edge_Gradient"

# Gaps to analyze (expanded list)
# Note: With 30 frames per lens:
#   Gap 1:  29 pairs, Gap 3:  27 pairs, Gap 5:  25 pairs
#   Gap 7:  23 pairs, Gap 10: 20 pairs, Gap 15: 15 pairs
#   Gap 17: 13 pairs, Gap 20: 10 pairs, Gap 23:  7 pairs
#   Gap 25:  5 pairs
...
def create_all_gaps_confusion_matrix(pair_data, output_path):
    ...
    # Calculate grid size based on number of gaps
    n_gaps = len(GAPS)
    n_cols = 4
    n_rows = (n_gaps + 1 + n_cols - 1) // n_cols  # +1 for summary panel
    ...
    # Summary panel in next available slot
    ax = axes[len(GAPS)]
    ax.axis('off')
...
def create_data_summary_table(pair_data, lens_data, output_path):
    ...
    # Calculate figure height based on number of gaps
    n_gaps = len(GAPS)
    fig_height = max(10, 2 + n_gaps * 0.8)
    ...
    summary_data.append(['', '', '', '', '', '', '', '', ''])
    lens_header_row = len(summary_data)
    ...
    # Style totals row (dynamically based on number of gaps)
    for j in range(9):
        table[(totals_row, j)].set_facecolor('#E8E8E8')
        table[(totals_row, j)].set_text_props(fontweight='bold')
        table[(lens_header_row, j)].set_facecolor('#D5E8D4')
        table[(lens_header_row, j)].set_text_props(fontweight='bold')
